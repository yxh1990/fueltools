...
function($, _, i18n, Backbone, React, models, utils, componentMixins, controls) {
    ...
    var HealthcheckCredentials = React.createClass({
        ...
        render: function() {
            ...
            //console.log(JSON.parse(this.props.cluster.get('cluster_setting')['cluster_setting'])['global']['keystone_admin_password']);
            var defaultpassword = this.props.cluster.get('cluster_setting') ? JSON.parse(this.props.cluster.get('cluster_setting')['cluster_setting'])['global']['keystone_admin_password']:"admin";
            //var cluster_setting=this.props.cluster.get('cluster_setting')?JSON.parse(this.props.cluster.get('cluster_setting')):null;
            if(this.props.cluster.get('cluster_type')!="1")
            {
              if(!cred['inputchange'])
              {
                cred['user'] = 'admin';
                cred['password'] = defaultpassword;
                cred['tenant'] = "admin";
              }
            }
            return (
