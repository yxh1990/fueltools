...
def modify_volumes_hook(role_mapping, node):
    ...
    #filter是一个方法列表
    #用role_mapping作为参数,循环被filters定义的方法处理
    #这些方法的参数都是role_mapping和node,返回值都为None
    for f in filters:
        f(role_mapping, node)
    return role_mapping
...
def get_node_spaces(node):
    ...
    release = defaultrelease.getRelease()
    role_mapping = release.volumes_metadata['volumes_roles_mapping']
    all_spaces =   release.volumes_metadata['volumes']


    if "nova-compute" in node.all_roles:
        volumes = role_mapping["compute"]
...
class Disk(object):
    ...
    def __init__(self, volumes, generator_method, disk_id, name,
                 ...
                 disk_extra=None,IsMountToBoot=True):
        """Create disk
        ...
        self.ismountboot=IsMountToBoot
    ...
    def get_size(self, size):
        ...
        #1.判断需要分配的磁盘空间是否大于剩余空间,若分配空间小于剩余空间
        #则返回分配空间,否则返回0
        #2.要用磁盘总剩余空间-此次分配空间=最新剩余空间
...
class VolumeManager(object):
    def __init__(self, node):
        ...
        self.disks = []  #node.meta['disks']

        fristdisk = sorted(node.meta['disks'], key=lambda i: i['name'])[0]
        IsMountToBoot = True
        for d in sorted(node.meta['disks'], key=lambda i: i['name']):
            ...
            #1.满足上面的条件后,然后再进行下面的判断
            #2.只有第一块disk的boot_is_raid设置为true,其他disk设置为false
            boot_is_raid = False
            if d != fristdisk:
               IsMountToBoot = False
    ...
    def call_generator(self, generator, *args):
        generators = {
            ...
            'calc_min_soft_size': lambda: gb_to_mb(10)
        }
